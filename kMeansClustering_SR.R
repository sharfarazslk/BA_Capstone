...
#Load Data
  ...
  median_income = read_excel("MedianZIP-3.xlsx")
...
#  category_1.1 = major_categories[(major_categories$MajorCategory==66),]
...
#Join Income information
  ...
  HH_income$X__1 = NULL
  category_1$P_Q = category_1$Dollars/category_1$Quantity
  #hist(category_1$P_Q, xlim = c(0,100), breaks=1000)
  
  #histogram
  ggplot(data=category_1, aes(category_1$P_Q)) + 
    geom_histogram(aes(y =..density..), 
                   breaks=seq(0, 100, by = 2),
                   col="black", 
                   fill="gray",
                   alpha = 1) +
    labs(title="Histogram of Women's Apparel P/Q",subtitle="Average Price Per Unit") +
    labs(x="P/Q", y="Density") + 
    xlim(c(0,100))

  View(describe(category_1$P_Q))

#Determine which percent of the dataset are excluded
  #outliers are any datapoints above 74, extremes are above 110
  category_1 %>% 
    filter(P_Q >= 139) %>% 
    summarise(n_extreme = n())
  category_1 %>% 
    summarise(n_extreme = n())
  #3% of data is extreme values above 110 P/Q (341223/11155316*100)
  #1.78% of data is extreme values above 139 P/Q (341223/11155316*100)
  ...
  write.table(summary_c1, "clipboard", sep="\t", row.names=FALSE)
  ...
  ggmap(map) + geom_point(
    ...
    data=temp2, 
    alpha=.75, na.rm = T, cex=0.0000001)+
    #scale_fill_hue()+
    ...
    ggtitle("Clusters Based on Median Income", subtitle = "Women's Apparel") + ylab("Latitude") + xlab("Longitude") 
  
  
  
  #Write csv
  write.csv(kmean_c1, file="~Category66Clusters.csv")
  ...
  #histogram
  ggplot(data=category_2, aes(category_2$P_Q)) + 
    geom_histogram(aes(y =..density..), 
                   breaks=seq(0, 100, by = 2),
                   col="black", 
                   fill="gray",
                   alpha = 1) +
    labs(title="Histogram of Home Decor P/Q",subtitle="Average Price Per Unit") +
    labs(x="P/Q", y="Density") + 
    xlim(c(0,100))
  
  View(describe(category_2$P_Q))
  ...
  category_1 %>% 
    summarise(n_total = n())
  #0.39% of data is extreme values above 100 P/Q (43788/11155316*100)
  
  #Bringing outliers P/Qdown to median of outliers
  outliers = category_2[(category_2$P_Q>=100.00),]
  outliers$P_Q = 100
  temp = category_2[(category_2$P_Q<100),] #FIXED-----
  ...
  temp2$Cluster[temp2$Cluster==2] = 40000
  temp2$Cluster[temp2$Cluster==3] = 85000
  temp2$Cluster[temp2$Cluster==4] = 120000
  temp2$Cluster = as.factor(temp2$Cluster)
...
#    scale_fill_hue()+
    ...
    theme(axis.text.x = element_text(angle = 60, vjust=0.15, size=8),
          ...
          legend.box.just = c("top"))+
    guides(colour = guide_legend(override.aes = list(size=1)))+
    ggtitle("Clusters Based on Median Income", subtitle = "Home Decor") + ylab("Latitude") + xlab("Longitude")
  
  #Write csv
  write.csv(kmean_c2, file="~Category30Clusters.csv")
  ...
  #histogram
  ggplot(data=category_3, aes(category_3$P_Q)) + 
    geom_histogram(aes(y =..density..), 
                   breaks=seq(0, 100, by = 2),
                   col="black", 
                   fill="gray",
                   alpha = 1) +
    labs(title="Histogram of Men's Apparel P/Q",subtitle="Average Price Per Unit") +
    labs(x="P/Q", y="Density") + 
    xlim(c(0,100))
  
  View(describe(category_3$P_Q))
  ...
  category_3 %>% 
    summarise(n_extreme = n())
  #2.57% of data is extreme values above 100 P/Q (50397/1958029*100)
  
  #Bringing outliers P/Qdown to median of outliers
  outliers = category_3[(category_3$P_Q>=100.00),]
  outliers$P_Q = 100
  temp = category_3[(category_3$P_Q<100),] #FIXED----
  ...
  temp2$Cluster[temp2$Cluster==1] = 40000
  temp2$Cluster[temp2$Cluster==2] = 90000
  temp2$Cluster[temp2$Cluster==3] = 65000
  temp2$Cluster[temp2$Cluster==4] = 140000
  temp2$Cluster = as.factor(temp2$Cluster)
  ...
  ggmap(map) + geom_point(
    ...
    theme(axis.text.x = element_text(angle = 60, vjust=0.15, size=8),
          ...
          legend.box.just = c("top"))+
    guides(colour = guide_legend(override.aes = list(size=1)))+
    ggtitle("Clusters Based on Median Income", subtitle = "Men's Apparel") + ylab("Latitude") + xlab("Longitude")
  
  #Write csv
  write.csv(kmean_c3, file="~Category43Clusters.csv")
